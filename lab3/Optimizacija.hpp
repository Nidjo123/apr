...
std::valarray<double> nelder_mead_simpleks(Funkcija &f, std::valarray<double> x_0, double pomak = 1, double alpha = 1, double beta = 0.5,
				double gamma = 2, double sigma = 0.5, double eps = 1e-6, bool verbose = false);

std::valarray<double> hooke_jeeves(Funkcija &f, std::valarray<double> x_0, std::valarray<double> dx, std::valarray<double> eps, bool verbose = false);
